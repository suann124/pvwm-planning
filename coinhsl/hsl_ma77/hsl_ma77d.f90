...
!*************************************************
  ...
  interface MA77_solve_fredholm
      module procedure MA77_solve_fredholm_double
  end interface
  ...
  interface MA77_lmultiply
      module procedure MA77_lmultiply_double
  end interface
...
! If matrix is non-singular, call ma77_solve
    if (keep%flag /= 4) then
       if (present(scale)) then
          call MA77_solve_double(nrhs,lx,x,keep,control,info,scale=scale)
       else
          call MA77_solve_double(nrhs,lx,x,keep,control,info)
       end if
       return
    end if
...
!     Form U'*X = Y
  do j = 1, p - nb, nb
    call dgemv('T',nb,n+1-j-nb,one,ap(jj+nbt),nb,y(j),1,one,z(j+nb),1)

    call dtpmv('U','T','N',nb,ap(jj),y(j),1)

    call daxpy(nb,one,y(j),1,z(j),1)
  ...
  if (n>=j+jb) then
    call dgemv('T',jb,n+1-j-jb,one,ap(jj+jbt),jb,y(j),1,one,z(j+jb),1)
  end if

  call dtpmv('U','T','N',jb,ap(jj),y(j),1)

  call daxpy(jb,one,y(j),1,z(j),1)
...
!                 < 0 The value of argument -info is not valid.
  ...
  jb = p - j + 1
  jbt = (jb*(jb+1_long))/2
  call dtpmv('U','N','N',jb,ap(jj),y(j),1)
  if (n>=j+jb) then
    call dgemv('N',jb,n+1-j-jb,one,ap(jj+jbt),jb,z(j+jb),1,one,y(j),1)
  end if
  ...
  do j = i, 1, -nb
    jd = jd + nb2
    jj = jj - jd
    call dtpmv('U','N','N',nb,ap(jj),y(j),1)
    call dgemv('N',nb,n+1-j-nb,one,ap(jj+nbt),nb,z(j+nb),1,one,y(j),1)
  end do
...
! .. Locals ..
    ...
    do j = 1, q, nb
      ...
      k1 = k + (jb*(jb+1_long))/2
      lj = lj - jb
      if (lj>0) call dgemv('N',lj,jb,one,a(k1),lj,y(j),1,one,z(j+jb),1)

      call dtpmv('L','N','U',jb,a(k),y(j),1)
...
! .. Locals ..
    ...
    do j = j1, 1, -nb
      ...
      k = k - lj*jb
      k1 = k - (jb*(jb+1_long))/2
      call dtpmv('L','T','U',jb,a(k1),y(j),1)

      if (lj>0) call dgemv('T',lj,jb,one,a(k),lj,y(j+jb),1,one,y(j),1)
      k = k1
      lj = lj + jb
    end do

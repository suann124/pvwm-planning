...
!             Version 2.0.0 for positive definite and indefinite matrices
...
! (3) globally change dcopy, daxpy, dnrm2, dgemv, dtpmv
!     to scopy, saxpy, snrm2, sgemv, stpmv   respectively.
...
! All integers are declared to be either short or long
  ...
  integer(short), parameter, private :: lup1 = huge(0_short)/4 !(do NOT
...
!           and real in-core buffers.
    ...
    real(wp) :: consist_tol = epsilon(one) ! used on call to ma77_solve_fredholm
!           to determine if system is consistent (singular case only)
...
!           of the current element/row (used by MA77_input_reals).
    ...
    integer(long)  :: lup ! huge(0_short)/4 (32-bit) or huge(0_long)/4 (64-bit)
...
! Check status parameter
    ...
    keep%lup = huge(0_long)/4
    if (control%bits == 32) keep%lup = huge(0_short)/4
...
! children (placed in keep%splitp)
     ...
     integer(short) :: store
     integer(short) :: i,j ! temporary variables
...
! Construct tree when all pivots are 1x1
    integer(short) :: i,ielt,ii,j,jdum,jj,k ! temporary variables
...
! keep%mvar holds largest value of nvar so far.
    if (keep%size(index) < 0) then
      ...
      if (keep%element_input) then
        ...
        if (.not. allocated(keep%map)) allocate (keep%map(0:keep%mvar),stat=st)
      else
        len = keep%mvar
...
!   write(*,*) 'entering MA77_factor with keep%status=', keep%status
    ...
    if (present(scale)) then
      call MA77_factor_solve_single(pos_def,keep,control,info,0,0,x,scale)
    else
       call MA77_factor_solve_single(pos_def,keep,control,info,0,0,x)
    end if
...
!    write (6,*) 'keep%index ',keep%index(1:4)
    ...
    if (keep%pos_def) then
      ...
      nfront_long = max(nfront_long,int(real(nfront_long,wp)*multiplier,long))
      lfa = (nfront_long*(nfront_long+nb+1)/2)
      if (lfa > keep%lup) info%flag = -30
      maxlen = int(real(keep%maxlen,wp)*multiplier)
    end if
...
! (pos definite case).
    ...
    integer(short) :: i,ic,j,jj,k ! temporary variables
...
! or assembles it into its parent node. cnode is the ic-th child of node.
    ...
    integer(short) :: i,j,jc,k,l ! temporary variables
...
!                            delayed pivots (only needed in indefinite case)
    real(wp), intent(in),optional :: scale(*)
    integer(short) :: i,ic,ii,j,jc,jj ! temporary variables
...
!             write (6,*) fa(ipp:ipp+lreals-1)
            ...
            keep%rtopmx = max(keep%rtopmx,rtop)
            deallocate (fa,stat=st)
            lfa = max(nfront_long,int(real(nfront_long,wp)*multiplier,long))
            lfa = (lfa*(lfa+nb+1))/2
...
!*************************************************************************
   subroutine MA77_resid_single(nrhs,lx,x,lresid,resid,keep,control,info, &
      anorm_bnd)
...
!       present when not expected
    ...
    integer(short) :: ir ! do loop variable
    integer(short) :: i ! temporary variable
    integer(short) :: ld ! size of array d
    integer(long)  :: lfa ! size of array fa
...
! Check status parameter
    ...
    if (st /= 0) go to 95
      ...
      if (local_job /= 2) then
        lfa = keep%maxfa
        if (keep%index(2) == 0) lfa = 1
        allocate (fa(lfa),stat=st)
        if (st /= 0) go to 95
      end if
      if (.not. keep%pos_def .and. local_job /= 1) then
        ld = 2*keep%maxelim_actual
        if (keep%index(2) == 0) ld = 1
...
! Perform work at node.
        call forward(node,nb,locfac,locint,maxlen,varlist,nrhs,lx,x, &
             xlocal,lfa,fa,lw54,w54,keep,info)
        if (info%flag < 0) exit
! If we have reached the root, we are done
        if (node == root) exit
...
! Visit each node in tree rooted at root in reverse depth-first search order
        ...
        do
          ...
          if (count(depth) == 0 .and. node > nelt) then
            if (keep%pos_def) then
               call back_posdef(node,nb,locend,maxlen,varlist,nrhs, &
                    lx,x,xlocal,lfa,fa,lw54,w54,keep,info)
            else
               call back_indef(node,nb,locend,locendi,maxlen,varlist,nrhs,  &
                    lx,x,xlocal,posdiag,ld,d,lfa,fa,ldiag_solve,keep,info)
            end if
            if (info%flag < 0) exit
            nchild = size(keep%tree(node)%child)
            if (nchild == 0) then
...
! Perform work at node.
        call diag_solve(node,locint,maxlen,varlist,nrhs,lx,x, &
             xlocal,posdiag,ld,d,keep,info)
        if (info%flag < 0) exit
! If we have reached the root, we are done
        if (node == root) exit
...
subroutine ma77_solve_fredholm_single(nrhs, flag_out, lx, x,   &
    ...
    integer(short), intent (in) :: lx
    real (wp), intent (inout) :: x(lx,2*nrhs) ! On entry, x must
      ! be set so that if i has been used to index a variable,
      ! x(i,j) is the corresponding component of the
      ! right-hand side for the jth system (j = 1,2,..., nrhs).
      ! On exit, x(1:n,1:nrhs) holds same solution as is computed by ma97_solve
      ! and, if flag_out*j)=.false., x(1:n,nrhs+j-1)
...
! For details of keep, control, info : see derived type description
    ...
    real(wp), intent(in), optional :: scale(:) ! if present,
!      must hold scaling factors. If scale was present on
!      call to ma77_factor, then scale must also be present here.

    real (wp), allocatable :: xlocal(:,:) ! temporary dense vector for
!              performing forward eliminations and back substitutions.
    real (wp), allocatable :: d(:)   ! used to hold diagonal entries
    real (wp), allocatable :: fa(:)  ! used to hold factor entries
    real (wp), allocatable :: tol(:) ! tolerance used to determine if system is
        ! consistent (singular case only)
...
!          an element list
    ...
    integer(short) :: nout ! output unit
    integer(short) :: nroot ! number of roots (components)
    integer(short) :: num_false ! number of entries of flag_out set to false
    integer(short) :: num_rhs_bwd ! number of rhs for back sub (equal to nrhs if
...
! Perform appropriate printing
    if (control%print_level >= 1 .and. control%unit_diagnostics >= 0) then
      write (control%unit_diagnostics,'(//a)') &
     ' Entering MA77_solve_fredholm with:'
      write (control%unit_diagnostics,'(a,4(/a,i12),(/a,i12/a,i12))') &
     ' control parameters (control%) :', &
     ' print_level         Level of diagnostic printing        = ', &
       control%print_level, &
...
! Check status parameter
    ...
    flag_out(1:nrhs) = .true.

! If matrix is non-singular, call ma77_solve
    if (keep%flag /= 4) then
       if (present(scale)) then
          call MA77_solve_single(nrhs,lx,x,keep,control,info,scale=scale)
       else
          call MA77_solve_single(nrhs,lx,x,keep,control,info)
       end if
       return
    end if
    ...
    maxlen = keep%maxlen
    maxlen = max(maxlen,keep%maxfrontb)
    ...
    allocate (xlocal(maxlen,2*nrhs),stat=st)
    if (st /= 0) go to 95
...
! At this point, array x holds partial solution vector(s).
    ...
    num_false = 0
! pass over tree checking for consistency of computed solution.
! if consistent, diag solve is performed
...
! Perform work at node.
        call diag_check(node,locint,maxlen,varlist,nrhs,lx,x, &
             xlocal,posdiag,ld,d,flag_out,num_false,tol,keep,info)
        if (info%flag < 0) exit
...
! Go to parent
    ...
    if (num_false.gt.0 .and. num_false.lt.nrhs) then
       ! make sure that cols nrhs+1:2*nrhs of x are defined
       ! (as will perform back sub on these cols even if means
       ! we are solving for some consistent cols twice).
      do irhs = 1,nrhs
         if (.not. flag_out(irhs)) cycle
         x(1:n,irhs+nrhs) = zero
      end do
    end if

! We must perform the back substitution.
...
! During back sub, x will be overwritten by solution.
! Back substitutions. Initialise locend/locendi to first free position
! after the end of the factor storage (we must read
! records in reverse order to the written order)
...
! Visit each node in tree rooted at root in reverse depth-first search order
        ...
        do
          cnode(depth) = node
          if (count(depth) == 0 .and. node > nelt) then
            call back_indef(node,nb,locend,locendi,maxlen,varlist,num_rhs_bwd, &
                 lx,x,xlocal,posdiag,ld,d,lfa,fa,ldiag_solve,keep,info)
            if (info%flag < 0) exit
            nchild = size(keep%tree(node)%child)
            if (nchild == 0) then
...
! If we have reached the root, we are done
   ...
   !  write (6,*) x(1:n,1)
    ...
    if (present(scale)) then
      do i = 1,n
        x(i,1:num_rhs_bwd) = scale(i)*x(i,1:num_rhs_bwd)
      end do
    end if
...
! Forms y=PLx or y=(PL)^Tx

    logical, intent (in) :: trans ! if set to .true. compute (PL)^TX;
!             otherwise, compute (PL)X
    integer(short), intent (in) :: k
    integer(short), intent (in) :: lx, ly
    real (wp), intent (inout) :: x(lx,k) ! On entry, x must
!             be set to hold the vector to be multiplied.
!             Only altered in case of trans = .true. and scale present.
...
!      call to ma77_factor , then scale should also be present here.
    ...
    real (wp), allocatable :: xlocal(:,:) ! temporary dense vector
    real (wp), allocatable :: zlocal(:,:) ! temporary dense vector
    real (wp), allocatable :: fa(:)  ! used to hold factor entries
    integer(short), allocatable :: varlist(:) ! used for variable lists
    integer(short), allocatable :: count(:) ! used for depth
!      first search of tree to keep track of level we are at
...
!          an element list
    ...
    integer(short) :: node ! node in tree
    integer(short) :: nout ! output unit
    integer(short) :: nrhs ! set to k (no. of colums to multiply)
    integer(short) :: nroot ! number of roots (components)
    integer(short) :: pnode ! parent node in tree
    integer(short) :: root ! root of tree
    integer(short) :: st ! stat parameter
...
! Perform appropriate printing
    if (control%print_level >= 1 .and. control%unit_diagnostics >= 0) then
      write (control%unit_diagnostics,'(//a)') &
     ' Entering MA77_lmultiply with:'
      write (control%unit_diagnostics,'(a,7(/a,i12))') &
     ' control parameters (control%) :', &
     ' print_level         Level of diagnostic printing        = ', &
       control%print_level, &
     ' unit_diagnostics    Unit for diagnostics                = ', &
...
! Check status parameter
    ...
    if (ly < n) then
      ...
      call MA77_print_iflag(keep,nout,info%flag)
      if (nout >= 0) write (nout,'(a,i8,a,i8)') &
    ' Increase ly from ', lx, ' to at least ', n
      return
     end if
    ...
    allocate (varlist(maxlen+1),stat=st)
    if (st /= 0) go to 95
    allocate (xlocal(maxlen,nrhs),zlocal(maxlen,nrhs),stat=st)
    if (st /= 0) go to 95
    ! ensuer xlocal is well defined.
    xlocal(1:maxlen,1:nrhs) = zero

    nroot = size(keep%roots)

    ! setting y to zero is needed in the trans=.false. case.
    ! Bu we also set it to zero in trans=.true. so that
    ! all entries of y are well defined (o.w. if i is not used to index
    ! a variable, y(i) will be undefined)
    y(1:n,1:nrhs) = zero

    if (.not. trans) then

! Form PLx. Mimic forward eliminations. Initialise locfac/locint to the start of
! the factor storage (skip diagonal entries that are stored first)
      locfac = keep%posfac + n
      if (.not. keep%pos_def) locfac = locfac + n
      locint = keep%posint
...
! Perform work at node.
        call lforward(node,nb,locfac,locint,maxlen,varlist,nrhs,lx,x,ly,y, &
             xlocal,zlocal,lfa,fa,keep,info)
        if (info%flag < 0) exit
! If we have reached the root, we are done
        if (node == root) exit
...
! Go to parent
      ...
      if (present(scale)) then
        do i = 1,n
           y(i,1:nrhs) = y(i,1:nrhs) / scale(i)
        end do
      end if

    else

! (PL)^Tx. Mimic back sub. Scale first (S^{-1}x)

      if (present(scale)) then
        do i = 1,n
           x(i,1:nrhs) = x(i,1:nrhs) / scale(i)
        end do
      end if
...
! Visit each node in tree rooted at root in reverse depth-first search order
        ...
        do
          ...
          if (count(depth) == 0 .and. node > nelt) then
            if (keep%pos_def) then
               call lback_posdef(node,nb,locend,maxlen,varlist,nrhs, &
                    lx,x,ly,y,xlocal,zlocal,lfa,fa,keep,info)
            else
               call lback_indef(node,nb,locend,locendi,maxlen,varlist,nrhs, &
                    lx,x,ly,y,xlocal,lfa,fa,keep,info)
            end if
            if (info%flag < 0) exit
            nchild = size(keep%tree(node)%child)
            if (nchild == 0) then
...
!*******************************
    ...
    subroutine forward(node,nb,locfac,locint,maxlen,varlist,nrhs,lx,x, &
      xlocal,lfa,fa,lw54,w54,keep,info)
! This subroutine performs forward eliminations at node.
! User-supplied rhs vectors are overwritten by the partial solution.

    integer(short), intent (in) :: node
    integer(short), intent (in) :: nb ! block size
    integer(long), intent (inout) :: locfac! initialised to start of factor
    integer(long), intent (inout) :: locint! initialised to start of integers
    integer(short), intent (in) :: maxlen
    integer(short), intent (inout) :: varlist(maxlen+1)! used for variable lists
    integer(short), intent (in) :: nrhs
    integer(short), intent (in) :: lx
    real (wp), intent (inout) :: x(lx,nrhs)
    real (wp), intent (inout) :: xlocal(maxlen,nrhs)
    integer(short) :: lw54 ! length of workarray w54
    real (wp), intent (inout) :: w54(lw54)
    integer(long) :: lfa ! length of workarray lfa
    real (wp), intent (inout) :: fa(lfa)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_info), intent (inout) :: info
    ...
    integer(long)  :: lq ! number of factor entries to read
...
! Read in required part of factor and do forward elimination ops.
          ...
          ii = 1
          do j = 1,nelim
            call of01_read(keep%index(2),locfac,lreals,fa(ii:ii+lreals-1), &
                 flag,keep%rdata,lp=-1)
            if (flag < 0) go to 485
...
!*******************************
    ...
    subroutine back_posdef(node,nb,locend,maxlen,varlist,nrhs,lx,x, &
      xlocal,lfa,fa,lw54,w54,keep,info)

! This subroutine performs backsubstitution on
! the subtree rooted at node (pos def case).

    integer(short), intent (in) :: node
    integer(short), intent (in) :: nb ! block size
    integer(long), intent (inout) :: locend ! first free position
!          after end of factor storage)
    integer(short), intent (in) :: maxlen
    integer(short), intent (inout) :: varlist(maxlen+1)! used for variable lists
    integer(short), intent (in) :: nrhs
...
! read required part of factor and do back sub. ops.
          ...
          ii = 1
          do j = 1,nelim
            call of01_read(keep%index(2),locend,lreals,fa(ii:ii+lreals-1), &
                flag,keep%rdata,lp=-1)
            if (flag < 0) go to 485
...
500   return
    ...
    end subroutine back_posdef


!*******************************

    subroutine back_indef(node,nb,locend,locendi,maxlen,varlist,nrhs,lx,x, &
      xlocal,posdiag,ld,d,lfa,fa,ldiag_solve,keep,info)

! This subroutine performs backsubstitution on
! the subtree rooted at node (indefinite case).

    integer(short), intent (in) :: node
    integer(short), intent (in) :: nb ! block size
    integer(long), intent (inout) :: locend ! first free position
!          after end of factor storage)
    integer(long), intent (inout) :: locendi ! first free position
!          after end of factor storage) (integer data)
    integer(short), intent (in) :: maxlen
    integer(short), intent (inout) :: varlist(maxlen+1)! used for variable lists
    integer(short), intent (in) :: nrhs
    ...
    real (wp), intent (inout) :: x(lx,nrhs)
    real (wp), intent (inout) :: xlocal(maxlen,nrhs)
    integer(long), intent (inout) :: posdiag ! start of diagonal data for node
    integer(short), intent (in) :: ld
    real (wp), intent (inout) :: d(ld)
    integer(long) :: lfa ! length of workarray lfa
    real (wp), intent (inout) :: fa(lfa)
    logical, intent(in) :: ldiag_solve ! controls whether a diagonal solve
!          is performed on the call to the backsubstitution.
    type (MA77_keep), intent (inout) :: keep
    type (MA77_info), intent (inout) :: info
...
500   return
    ...
    end subroutine back_indef

!*******************************

    subroutine lforward(node,nb,locfac,locint,maxlen,varlist,nrhs,lx,x,ly,y, &
      xlocal,zlocal,lfa,fa,keep,info)
...
! Computes y = PLx.
    ...
    integer(short), intent (in) :: lx
    real (wp), intent (in) :: x(lx,nrhs)
    integer(short), intent (in) :: ly
    real (wp), intent (inout) :: y(ly,nrhs)
        !!!! note : as we currently loop over the right-handsides
        ! and treat each one separately, xlocal and zlocal only need
        ! to be rank-one arrays of size maxlen
    real (wp), intent (inout) :: xlocal(maxlen,nrhs)
    real (wp), intent (inout) :: zlocal(maxlen,nrhs)
    integer(long) :: lfa ! length of workarray lfa
    real (wp), intent (inout) :: fa(lfa)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_info), intent (inout) :: info
...
! Read integer data from file
          call of01_read(keep%index(1),loc,nfront,varlist,flag, &
               keep%idata,lp=-1)
          if (flag < 0) go to 480
! Copy part of array x corresponding to the variables
! that are in varlist into local array xlocal
          do i = 1, nelim
            jvar = varlist(i)
            xlocal(i,1:nrhs) = x(jvar,1:nrhs)
...
! Integers are held in keep%imain
        ...
        if (keep%index(2) >= 0) then
! Read in required part of factor and do multiply.
          lreals = nfront
          ii = 1
          do j = 1,nelim
            call of01_read(keep%index(2),locfac,lreals,fa(ii:ii+lreals-1), &
          ...
          do j = 1,nrhs
            call ma54_multiplyL1(nfront,nelim,nb,fa,xlocal(:,j),zlocal(:,j),&
                 flag)
          end do
...
! Factor entries are held in keep%rmain
          ...
          do j = 1,nrhs
            call ma54_multiplyL1(nfront,nelim,nb, &
                 keep%rmain(locfac:locfac+lq-1),xlocal(:,j),zlocal(:,j),flag)
          end do
          locfac = locfac + lq
        end if
      ...
      if (keep%index(1) >= 0) then
        do i = 1, nfront
          jvar = varlist(i)
          y(jvar,1:nrhs) = y(jvar,1:nrhs) + zlocal(i,1:nrhs)
        end do
      else
        do i = 1, nfront
          jvar = keep%imain(loc+i-1)
...
! read required part of factor and do forward eliminations
          ...
          do j = 1,nrhs
            call ma64_multiplyL1(nfront,nelim,nb,xlocal(:,j),zlocal(:,j), &
                 flag,fa,lq)
          end do
          do i = 1, nfront
            jvar = varlist(i)
...
! factors are in keep%rmain
          do j = 1,nrhs
            call ma64_multiplyL1(nfront,nelim,nb,xlocal(:,j),zlocal(:,j),flag, &
                 keep%rmain(locfac:locfac+lq-1),lq)
          end do
          locfac = locfac + lq
          do i = 1, nfront
...
500   return
    ...
    end subroutine lforward
!**************************************

    subroutine lback_posdef(node,nb,locend,maxlen,varlist,nrhs,lx,x, &
      ly,y,xlocal,zlocal,lfa,fa,keep,info)

! Computes y = (PL)^T x (pos def case).
...
!          after end of factor storage)
    ...
    integer(short), intent (in) :: ly
    real (wp), intent (inout) :: y(ly,nrhs)
    real (wp), intent (out) :: xlocal(maxlen,nrhs)
    real (wp), intent (out) :: zlocal(maxlen)
    integer(long) :: lfa ! length of workarray lfa
    real (wp), intent (inout) :: fa(lfa)
    type (MA77_keep), intent (inout) :: keep
    type (MA77_info), intent (inout) :: info
...
! copy x into local array
        ...
        locend = locend - lq
        if (keep%index(2) >= 0) then
! read required part of factor and multiply ops.
          lreals = nfront
          ii = 1
          do j = 1,nelim
            call of01_read(keep%index(2),locend,lreals,fa(ii:ii+lreals-1), &
          ...
          locend = locend - lq
            do j = 1,nrhs
              call ma54_multiplyLT1(nfront,nelim,nb,fa,xlocal(:,j),zlocal,flag)
            end do
        else
            do j = 1,nrhs
              call ma54_multiplyLT1(nfront,nelim,nb, &
                   keep%rmain(locend:locend+lq-1),xlocal(:,j),zlocal,flag)
            end do
       end if

! copy part of xlocal back into y
      if (keep%index(1) >= 0) then
        do i = 1, nelim
          jvar = varlist(i)
          y(jvar,1:nrhs) = xlocal(i,1:nrhs)
        end do
      else
        do i = 1, nelim
          jvar = keep%imain(loc+i-1)
...
500   return
    ...
    end subroutine lback_posdef

!*******************************

    subroutine lback_indef(node,nb,locend,locendi,maxlen,varlist,nrhs,lx,x, &
      ly,y,xlocal,lfa,fa,keep,info)
...
! reset locend
            locend = lcopy
              ...
              do j = 1,nrhs
                call ma64_multiplyLT1(nfront,nelim,nb,xlocal(:,j),flag,fa,lq)
              end do

          else
              do j = 1,nrhs
                call ma64_multiplyLT1(nfront,nelim,nb,xlocal(:,j),flag, &
                   keep%rmain(locend:locend+lq-1),lq)
              end do
          end if
...
500   return
    ...
    end subroutine lback_indef

!*******************************

    subroutine diag_check(node,locint,maxlen,varlist,nrhs,lx,x, &
             xlocal,posdiag,ld,d,flag_out,num_false,tol,keep,info)

! indefinite singular case. check consistency after forward solve
! and perform diag solve.

    integer(short), intent (in) :: node
    integer(long), intent (inout) :: locint ! initialised to start of integers
    integer(short), intent (in) :: maxlen
    integer(short), intent (inout) :: varlist(maxlen+1) ! variable lists
    integer(short), intent (in) :: nrhs
    integer(short), intent (in) :: lx
    real (wp), intent (inout) :: x(lx,2*nrhs)
    real (wp), intent (inout) :: xlocal(maxlen,nrhs)
    integer(long), intent (inout) :: posdiag
    integer(short), intent (in) :: ld
    real (wp), intent (inout) :: d(ld)
    logical, intent (inout) :: flag_out(nrhs)
    integer(short), intent (inout) :: num_false ! number of inconsistent rhs
...
! file not in use
        nelim = keep%imain(locint+nfront)
        if (nelim == 0) go to 100
! copy partial solution which is held in x into local array
        do i = 1, nfront
          jvar = keep%imain(locint+i-1)
          xlocal(i,1:nrhs) = x(jvar,1:nrhs)
        end do
      end if
...
! data held in keep%rmain
        d(1:dreals) = keep%rmain(posdiag:posdiag+dreals-1)
      end if
      posdiag = posdiag + dreals
      ...
      do irhs = 1,nrhs
        ...
        do
          if (j > nelim) exit
          if (d(2*j) .eq. zero) then
! 1x1 pivot
            temp = xlocal(j,irhs)
            if (flag_out(irhs) .and. d(2*j-1).eq.zero) then
              ! check if rhs is inconsistent (but only if consistent so far)
              if (abs(temp) .gt. tol(irhs)) then
                flag_out(irhs) = .false.
                num_false = num_false + 1
                ! set partial solution to be zero for this rhs,
                ! except for entry corresponding to inconsistency
                x(1:keep%n,irhs+nrhs) = zero
                if (keep%index(1) >= 0) then
                  jvar = varlist(j)
                else
                  jvar = keep%imain(locint+j-1)
                end if
                x(jvar,irhs+nrhs) = one
              end if
            end if
            xlocal(j,irhs) = temp * d(2*j-1)
            j = j + 1
          else
! 2x2 pivot ... this 2x2 is not singular so perform diag solve
            temp = xlocal(j,irhs)*d(2*j-1) + xlocal(j+1,irhs)*d(2*j)
            xlocal(j+1,irhs) = xlocal(j,irhs)*d(2*j) + xlocal(j+1,irhs)*d(2*j+1)
            xlocal(j,irhs) = temp
            j = j + 2
          end if
        end do
...
!*******************************
    ...
    subroutine diag_solve(node,locint,maxlen,varlist,nrhs,lx,x, &
      xlocal,posdiag,ld,d,keep,info)
! This subroutine performs diagonal solve at node.

    integer(long), intent (inout) :: locint ! start of integer data for node
    integer(short), intent (in) :: maxlen
    integer(short), intent (in) :: node
    integer(short), intent (inout) :: varlist(maxlen+1) ! variable lists
    integer(short), intent (in) :: nrhs
...
! at least one array is being used.
! Try and write the integers to file. Have to do this in a loop
! since the amount of data to write is a long integer but each call
! to of01_write allows only short integer write.
        if (keep%index(1) < 0) then
          keep%index(1) = -keep%index(1)
          loc1 = 1
          do jlong = 1,keep%ifree-1,huge(lw)
            lw = int( min(int(huge(lw),long),keep%ifree-1-jlong))
            call of01_write(keep%index(1),loc1,lw,keep%imain, &
                flag,keep%idata,lp=-1)
            if (flag < 0) then
              info%iostat = keep%idata%iostat
...
! Try switching reals to a file
        if (keep%index(2) < 0) then
          keep%index(2) = -keep%index(2)
          loc1 = 1
          do jlong = 1,keep%posfac-1,huge(lw)
            lw = int( min(int(huge(lw),long),keep%posfac-1-jlong) )
            call of01_write(keep%index(2),loc1,lw,keep%rmain, &
               flag,keep%rdata,lp=-1)
            if (flag < 0) then
              info%iostat = keep%rdata%iostat
...
!             computing matrix norm
    integer(short) :: i,ic,j,jj ! temporary variables
...
!       call saxpy(length,one,keep_array(loc),1,read_array,1)
     else
       call scopy(length,keep_array(loc),1,read_array,1)
     end if
   end if
...
! Local scalars
   ...
   integer(long) :: klong,lkeep,loc1,lw,lw1_long,nwrite_long
...
! Try and copy from write_array into keep_array
   ...
   if (loc+length-1 <= lkeep) then
     call scopy(length,write_array,1,keep_array(loc),1)
     return
   end if
...
! Remember that lw is a long integer so split this copy into chunks
       if (lw > 0) then
         lw1 = lup1
         nwrite_long = lw/lw1
         nwrite = int(nwrite_long)
         loc1 = 1
         do i = 1,nwrite
           call scopy(lw1,keep_array(loc1),1,temp(loc1),1)
           loc1 = loc1 + lw1
         end do
         lw1_long = lw1
       ...
       if (st == 0) then
         ...
         if (lw > 0) then
           ...
           loc1 = 1
           do i = 1,nwrite
             call scopy(lw1,temp(loc1),1,keep_array(loc1),1)
             loc1 = loc1 + lw1
           end do
           lw1_long = lw1
           lw1 = int((lw - nwrite*lw1_long),short)
...
! Local scalars
   ...
   integer(long) :: ilong,klong,kk,lkeep,lw1_long,loc1,lw,nwrite_long
...
! reallocate keep_array with longer length
       ...
       do ilong= klong+1,lw, 7_long
         temp(ilong) = keep_array(ilong)
         temp(ilong+1) = keep_array(ilong+1)
         temp(ilong+2) = keep_array(ilong+2)
         temp(ilong+3) = keep_array(ilong+3)
         temp(ilong+4) = keep_array(ilong+4)
         temp(ilong+5) = keep_array(ilong+5)
         temp(ilong+6) = keep_array(ilong+6)
       end do
!      temp(1:lw) = keep_array(1:lw)
       ...
       if (st == 0) then
         ...
         do ilong = klong+1, lw, 7_long
           keep_array(ilong) = temp(ilong)
           keep_array(ilong+1) = temp(ilong+1)
           keep_array(ilong+2) = temp(ilong+2)
           keep_array(ilong+3) = temp(ilong+3)
           keep_array(ilong+4) = temp(ilong+4)
           keep_array(ilong+5) = temp(ilong+5)
           keep_array(ilong+6) = temp(ilong+6)
         end do
...
!!! This is using the ma54 solve code, adapted for forming the product
!   y = Lx
!   (we need to retain ma54 data structures)


subroutine ma54_multiplyL1(n,p,nb,ap,y,z,info)

! Computes            y =    ( L ) x
!                            ( M )
!          given a partial Cholesky factorization in blocked hybrid format.
!          L is non-unit triangular
  ...
  real (wp), intent (in) :: ap(0:(p*(n*2-p+1_long))/2-1) ! Holds the matrix.
  real (wp), intent (inout) :: y(n) ! holds x
!                   on entry and is overwritten.
  real (wp), intent (out) :: z(n) ! holds y on exit
...
!     Form U'*X = Y
  do j = 1, p - nb, nb
    call sgemv('T',nb,n+1-j-nb,one,ap(jj+nbt),nb,y(j),1,one,z(j+nb),1)

    call stpmv('U','T','N',nb,ap(jj),y(j),1)

    call saxpy(nb,one,y(j),1,z(j),1)
  ...
  if (n>=j+jb) then
    call sgemv('T',jb,n+1-j-jb,one,ap(jj+jbt),jb,y(j),1,one,z(j+jb),1)
  end if

  call stpmv('U','T','N',jb,ap(jj),y(j),1)

  call saxpy(jb,one,y(j),1,z(j),1)

end subroutine ma54_multiplyL1
...
!   (we need to retain ma54 data structures)
  ...
  subroutine ma54_multiplyLT1(n,p,nb,ap,y,z,info)
...
!          L is non-unit triangular
  ...
  real (wp), intent (in) :: ap(0:(p*(n*2-p+1_long))/2-1) ! Holds the matrix.
  real (wp), intent (inout) :: y(n) ! holds x
!              on entry and is overwritten by y.
  real (wp), intent (out) :: z(n) ! holds copy of y as is on input
  integer, intent (out) :: info ! set to one of these values:
!                   0 Successful solution.
!                 < 0 The value of argument -info is not valid.
  ...
  jb = p - j + 1
  jbt = (jb*(jb+1_long))/2
  call stpmv('U','N','N',jb,ap(jj),y(j),1)
  if (n>=j+jb) then
    call sgemv('N',jb,n+1-j-jb,one,ap(jj+jbt),jb,z(j+jb),1,one,y(j),1)
  end if
  ...
  do j = i, 1, -nb
    jd = jd + nb2
    jj = jj - jd
    call stpmv('U','N','N',nb,ap(jj),y(j),1)
    call sgemv('N',nb,n+1-j-nb,one,ap(jj+nbt),nb,z(j+nb),1,one,y(j),1)
  end do

end subroutine ma54_multiplyLT1

!*******************************************
!!! This is using the ma64 solve code, adapted for forming the product
!   y = Lx
!   (we need to retain ma64 data structures)

  subroutine ma64_multiplyL1(n,q,nb,y,z,flag,a,ll)
...
!                                format.
    real (wp), intent (inout) :: y(n) ! y holds x
!                           on entry and is overwritten.
    real (wp), intent (out) :: z(n) ! z holds y on exit
...
! .. Locals ..
    ...
    integer(long) k, k1 ! Position in array
    ...
    do j = 1, q, nb
      ...
      k1 = k + (jb*(jb+1_long))/2
      lj = lj - jb
      if (lj>0) call sgemv('N',lj,jb,one,a(k1),lj,y(j),1,one,z(j+jb),1)

      call stpmv('L','N','U',jb,a(k),y(j),1)
  ...
  end subroutine ma64_multiplyL1
...
!   (we need to retain ma64 data structures)
  ...
  subroutine ma64_multiplyLT1(n,q,nb,y,flag,a,ll)
...
! .. Locals ..
    ...
    do j = j1, 1, -nb
      ...
      k = k - lj*jb
      k1 = k - (jb*(jb+1_long))/2
      call stpmv('L','T','U',jb,a(k1),y(j),1)

      if (lj>0) call sgemv('T',lj,jb,one,a(k),lj,y(j+jb),1,one,y(j),1)
      k = k1
      lj = lj + jb
    end do

  end subroutine ma64_multiplyLT1
!*******************************************

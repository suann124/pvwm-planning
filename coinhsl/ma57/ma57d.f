...
C  ICNTL(6) = 4     hsl_metis ordering from MeTiS package.
...
C Dummy call with 1 by 1 matrix
        ...
        KEEP(IPE+1) = 2
        KEEP(IFCT)  = 1
        CALL hsl_metis(1,KEEP(IPE),KEEP(IFCT),METFTN,METOPT,
     *                    KEEP(NSTK),KEEP(PERM))
C Flag set if dummy code for hsl_metis has been used
...
C Must test METIS ordering now ... ugh
     ...
     +              KEEP(IW),IWFR,ICNTL,INFO)
                ...
                DO 91 I = 1,N
                  ...
                  ENDIF
   91           CONTINUE
   92     CALL hsl_metis(N,KEEP(IPE),KEEP(IFCT),METFTN,METOPT,
     *                      KEEP(NSTK),KEEP(PERM))
...
C The rest are set to default values
              ...
              ENDIF
  102       CONTINUE
  103     CALL hsl_metis(N,KEEP(IPE),KEEP(IFCT),METFTN,METOPT,
     *                      KEEP(NSTK),KEEP(PERM))
            GO TO 111
          ENDIF
